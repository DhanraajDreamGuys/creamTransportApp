...
public class AddNewRunsheet extends AppCompatActivity implements View.OnClickListener, CompoundButton.OnCheckedChangeListener {
    Toolbar mToolbar;
    ...
    String checkedExpress = "", checkedNodanger = "", checkedHasDanger = "", checkedProduct = "", checkedItems = "", trailers = "", selectedId = "";
    CustomProgressDialog mCustomProgressDialog;
    private static String TAG = AddNewRunsheet.class.getName();
    int count = 0;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_new_runsheet);
    ...
    private void intiWidgets() {
        ...
        mToolbar.setTitle(getString(R.string.tool_add_runsheet_title));
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.ANT_TV_choose_driver) {
            Constants.AdminMenu.getDrivers(AddNewRunsheet.this, mChooseDriver);
        } else if (v.getId() == R.id.ANT_TV_from) {
            Constants.AdminMenu.getLocations(AddNewRunsheet.this, mFrom, Constants.FromString);
        } else if (v.getId() == R.id.ANT_TV_to) {
            Constants.AdminMenu.getLocations(AddNewRunsheet.this, mTo, Constants.ToString);
        } else if (v.getId() == R.id.ANT_TV_date) {
            Constants.AdminMenu.getFromDate(AddNewRunsheet.this, mDate);
        } else if (v.getId() == R.id.ANT_TV_set_date) {
            Constants.AdminMenu.getFromDate(AddNewRunsheet.this, mSetDate);
    ...
    private void addNewTrip() {
        if (mChooseDriver.getText().toString().isEmpty()) {
            ...
            Toast.makeText(AddNewRunsheet.this, getString(R.string.no_internet_connection), Toast.LENGTH_SHORT).show();
        } else {
            mCustomProgressDialog.showDialog();
            ApiInterface apiService =
                    ApiClient.getClient().create(ApiInterface.class);
            Call<AddTripAPI.AddTripResponse> loginCall = apiService.addNewRunsheet(sendValueWithRetrofit());
            loginCall.enqueue(new Callback<AddTripAPI.AddTripResponse>() {
                ...
                public void onResponse(Call<AddTripAPI.AddTripResponse> call, Response<AddTripAPI.AddTripResponse> response) {
                    ...
                    if (response.body().getMeta().equals(Constants.SUCCESS)) {
                        Toast.makeText(AddNewRunsheet.this, response.body().getMessage(), Toast.LENGTH_SHORT).show();
    ...
    private HashMap<String, String> sendValueWithRetrofit() {
        ...
        params.put(Constants.PARAMS_USER_ID, Constants.driverList.get(adapterPosition).getId());
        ...
        params.put(Constants.PARAMS_CODRIVER, mDriver.getText().toString());
        ...
        params.put(Constants.PARAMS_TO, Constants.countries.get(To).getId());
        params.put(Constants.PARAMS_ADMIN_COMMENT, mComments.getText().toString());
        if (!selectedId.isEmpty())
            params.put(Constants.PARAMS_LOCATION, selectedId);
