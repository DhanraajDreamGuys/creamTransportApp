...
public class EditPBManagement extends AppCompatActivity implements View.OnClickListener {
    Toolbar mToolbar;
    public static String TAG = EditPBManagement.class.getName();
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_management_edit_phonebook);
    ...
    private void editPBTruck() {
        if (mName.getText().toString().isEmpty()) {
            ...
            Toast.makeText(EditPBManagement.this, getString(R.string.no_internet_connection), Toast.LENGTH_SHORT).show();
        } else {
            mCustomProgressDialog.showDialog();
            ApiInterface apiService =
                    ApiClient.getClient().create(ApiInterface.class);
            Call<UpdateUsersAPI.UpdateUsersResponse> loginCall = apiService.editPBManagement(sendValueWithRetrofit());
            loginCall.enqueue(new Callback<UpdateUsersAPI.UpdateUsersResponse>() {
                ...
                public void onResponse(Call<UpdateUsersAPI.UpdateUsersResponse> call, Response<UpdateUsersAPI.UpdateUsersResponse> response) {
                    ...
                    if (response.body().getMeta().equals(Constants.SUCCESS)) {
                        Toast.makeText(EditPBManagement.this, response.body().getMessage(), Toast.LENGTH_SHORT).show();
                        finish();
                    } else {
                        Toast.makeText(EditPBManagement.this, response.body().getMessage(), Toast.LENGTH_SHORT).show();
                    }
    ...
    private void initWidgets() {
        ...
        mToolbar.setTitle(getString(R.string.toolbar_edit_pb_management_title));
        mToolbar.setTitleTextColor(Color.WHITE);

        mName = (EditText) findViewById(R.id.AMEPB_ET_name);
        mAddress = (EditText) findViewById(R.id.AMEPB_ET_address);
        mPhoneNo = (EditText) findViewById(R.id.AMEPB_ET_phoneno);
        mManager = (EditText) findViewById(R.id.AMEPB_ET_position);
        mStatus = (SwitchCompat) findViewById(R.id.AMEPB_SC_status);
    }
    ...
    private void callStatusApi(boolean status) {
        ...
        if (!isNetworkAvailable(this)) {
            ...
            ApiInterface apiService =
                    ApiClient.getClient().create(ApiInterface.class);
            Call<UpdateUsersAPI.UpdateUsersResponse> loginCall = apiService.editManagementStatus(mData.getId(), value);
